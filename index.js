// Script untuk button new dan rating
...
// Script untuk mengecek apakah sudah login atau tidak, NOTE : Belum di hubungkan dengan database
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (activeNav === 'navigation2') {
        navigation1.style.display = 'none';
        navigation2.style.display = 'flex';
    } else {
        navigation1.style.display = 'flex';
        navigation2.style.display = 'none';
    }

    function switchToNavigation2() {
        navigation1.style.display = 'none';
        navigation2.style.display = 'flex';
        localStorage.setItem('activeNavigation', 'navigation2');
    }

    function switchToNavigation1() {
        navigation1 .style.display = 'flex';
        navigation2.style.display = 'none';
        localStorage.setItem('activeNavigation', 'navigation1');
    }

    signInButton.addEventListener('click', switchToNavigation2);
    registerButton.addEventListener('click', switchToNavigation2);
    toggleNavButton.addEventListener('click', switchToNavigation1);
});

// Script untuk community
document.addEventListener("DOMContentLoaded", () => {
    ...
    const samplePosts = [
        "Selamat datang di komunitas! Silakan bagikan pendapat Anda.",
        "Disini adakah yang punya rekomendasi film bagus?",
        "Tadi aku baru namatin game, gamenya seru banget >.< yang main juga sini dong kita mabar wkwkwk!",
        "Guys, mau nanya jadi kan aku mau nonton film, tapi aku takut filmnya ga sesuai ekspektasi aku :( yang sudah nonton spill dong filmnya worth it atau ngga?",
        "Ada yang punya rekomen game seru untuk mabar?",
        "Film yang barusan gue tonton epic banget, kalo menurut kalian gmn?",
        "Saya ingin tahu pendapat kalian tentang game ini, adakah yang sudah pernah main?",
        "Film komedi terbaik tahun ini menurut kalian apa?",
        "Guys aku baru download game, yang pernah main boleh kasih tutor ga?",
        "Spill pengalaman menonton film kalian! Apa film terakhir yang kalian tonton?"
    ];
    ...
    samplePosts.forEach(post => addPost(post));
